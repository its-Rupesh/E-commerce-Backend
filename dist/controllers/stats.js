    ...
    try {
        ...
        if (myCache.has("admin-stats")) {
            ...
            const lastTransactionPromise = Order.find({})
                ...
                .limit(4);
            const [thisMonthProducts, lastMonthProducts, thisMonthUser, lastMonthUser, thisMonthOrder, lastMonthOrder, ProductCount, UserCount, allOrder, lastSixMonthOrder, categories, femaleUserCount, lastTransaction,] = await Promise.all([
            ...
            stats = {
                ...
                product: calaculatePercentage(thisMonthProducts.length, lastMonthProducts.length),
                user: calaculatePercentage(thisMonthUser.length, lastMonthUser.length),
                order: calaculatePercentage(thisMonthOrder.length, lastMonthOrder.length),
                count,
                chart: {
...
const getLineStats = async (req, res, next) => {
    try {
        let charts;
        ...
        if (myCache.has(key)) {
            ...
            const twelveMonthUsrsPromise = User.find({
                ...
                .select("createdAt")
                .lean();
            const [twelveMonthProduct, twelveMonthUser, twelveMonthOrders] = await Promise.all([
